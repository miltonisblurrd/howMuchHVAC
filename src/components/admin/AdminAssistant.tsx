...
const suggestions = [
  ...
  "Who hasn't paid?",
  "What's on the schedule Thursday?",
  "Summarize new leads",
...
export function AdminAssistant() {
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: "assistant",
      text: "Hey Andy ? I'm your How Much? ops assistant (demo). Ask me about cash, jobs, leads, or have me draft follow-ups.",
    },
  ]);
  ...
  return (
    <div className="flex h-[min(70vh,640px)] flex-col overflow-hidden rounded-2xl border border-hm-line bg-white shadow-sm">
      <div className="flex items-center gap-3 border-b border-hm-line px-5 py-4">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-hm-red text-white">
          <Sparkles className="h-5 w-5" />
        </div>
        <div>
          <p className="font-display font-bold text-hm-charcoal">How Much? AI</p>
          <p className="text-xs text-hm-muted">Demo assistant ? show & tell for Andy</p>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-5 py-4">
        {messages.map((m, i) => (
          <div
            ...
            className={
              m.role === "user"
                ? "ml-auto max-w-[85%] rounded-2xl bg-hm-red px-4 py-3 text-sm text-white whitespace-pre-wrap"
                : "max-w-[90%] rounded-2xl bg-hm-fog px-4 py-3 text-sm text-hm-charcoal whitespace-pre-wrap"
            }
          >
            {m.text}
          </div>
        ...
        {typing && (
          <div className="rounded-2xl bg-hm-fog px-4 py-3 text-sm text-hm-muted">
            Thinking?
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div className="border-t border-hm-line p-4">
        <div className="mb-3 flex flex-wrap gap-2">
          {suggestions.map((s) => (
            <button
              ...
              type="button"
              onClick={() => send(s)}
              className="rounded-full border border-hm-line bg-hm-fog px-3 py-1 text-xs font-semibold text-hm-charcoal hover:border-hm-red/40"
            >
              {s}
            </button>
          ))}
