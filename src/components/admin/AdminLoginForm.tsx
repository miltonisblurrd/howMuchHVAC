...
export function AdminLoginForm({ authEmailsEnabled }: { authEmailsEnabled: boolean }) {
        ...
        <div className="mt-8 rounded-2xl bg-white p-8 text-hm-charcoal shadow-[0_32px_80px_-24px_rgba(0,0,0,0.55)] ring-1 ring-black/5">
          ...
          {!authEmailsEnabled ? (
            <div className="mt-6 rounded-xl bg-amber-50 p-4 text-sm text-amber-950">
              ...
              <p className="mt-1">
                Auth emails are off until Resend SMTP is connected, so this project doesn?t get
                locked for bounces. Marketing site and lead capture still work.
              </p>
            </div>
          ) : sent ? (
...
export function AdminLoginFormBoundary({
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-hm-ink" />}>
      <AdminLoginForm authEmailsEnabled={authEmailsEnabled} />
    </Suspense>
  );
}
