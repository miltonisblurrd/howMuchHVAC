...
export function PortalLoginForm({ authEmailsEnabled }: { authEmailsEnabled: boolean }) {
        ...
        <div className="mt-8 rounded-2xl bg-white p-8 text-hm-charcoal shadow-[0_32px_80px_-24px_rgba(0,0,0,0.55)] ring-1 ring-black/5">
          ...
          {!authEmailsEnabled ? (
            <div className="mt-6 rounded-xl bg-amber-50 p-4 text-sm text-amber-950">
              ...
              <p className="mt-1">
                Portal login emails are off until Resend is connected, so we don?t bounce mail on
                Supabase?s shared sender. Call Andy and we?ll follow up directly.
              </p>
            </div>
          ) : sent ? (
...
export function PortalLoginFormBoundary({
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-hm-ink" />}>
      <PortalLoginForm authEmailsEnabled={authEmailsEnabled} />
    </Suspense>
  );
}
